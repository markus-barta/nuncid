...
@MainActor enum UpdateChecks {
    static func run() -> [String] {
        ...
        expect(UpdateCheckCadence.weekly.interval == 604_800 && UpdateCheckCadence.hourly.interval == 3_600 && UpdateCheckCadence.developing.interval == 300, "week, hour, and testing intervals")
        expect(UpdateSchedule.visible(developerMode: false) == [.weekly, .daily], "public schedule is week and day")
        expect(UpdateSchedule.visible(developerMode: true) == [.weekly, .daily, .hourly, .developing], "developer schedule adds the testing intervals")
        expect(UpdateSchedule.afterDisablingDeveloper(.developing) == .daily && UpdateSchedule.afterDisablingDeveloper(.hourly) == .daily && UpdateSchedule.afterDisablingDeveloper(.weekly) == .weekly, "leaving developer mode restores a public interval")
        defaults.set(true, forKey: DeveloperMode.key)
        defaults.set(UpdateCheckCadence.developing.rawValue, forKey: UpdateCheckCadence.storageKey)
        expect(AppUpdater(startingUpdater: false, defaults: defaults).cadence == .developing, "developer mode keeps a 5-minute schedule")
        let closed = AppUpdater(startingUpdater: false, defaults: defaults)
