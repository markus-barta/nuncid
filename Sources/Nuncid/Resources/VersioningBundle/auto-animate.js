// INSPR-421: reproducible lifecycle guards; see scripts/vendor-auto-animate.mjs.
/**
...
function stillOwned(el) { const parent = getTarget(el) || el.parentElement; return parents.has(el) || enabled.has(el) || parents.has(parent) || enabled.has(parent); }
/**
 ...
 */
function poll(el) {
    clearTimeout(pollStarts.get(el));
    pollStarts.set(el, setTimeout(() => {
        pollStarts.delete(el);
        if (!stillOwned(el)) return;
...
function autoAnimate(el, config = {}) {
    ...
    const controller = Object.freeze({
        ...
        destroy: () => {
            ...
            forEach(el, (node) => {
                clearTimeout(pollStarts.get(node));
                pollStarts.delete(node);
                // unobserve resize
