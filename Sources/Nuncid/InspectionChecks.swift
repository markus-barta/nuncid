...
enum InspectionChecks {
    static func run() -> [String] {
        ...
        let anchored = CGRect(x: 400, y: 400, width: 400, height: 300)
        let grown = PanelPlacement.topRightAnchored(current: anchored, newSize: CGSize(width: 500, height: 360), visibleFrame: CGRect(x: 0, y: 0, width: 1_600, height: 1_000))
        check(grown.x + 500 == anchored.maxX && grown.y + 360 == anchored.maxY, "zoom keeps the top-right corner fixed")
        let shrunk = PanelPlacement.topRightAnchored(current: anchored, newSize: CGSize(width: 280, height: 180), visibleFrame: CGRect(x: 0, y: 0, width: 1_600, height: 1_000))
        check(shrunk.x + 280 == anchored.maxX && shrunk.y + 180 == anchored.maxY, "zoom out keeps the top-right corner fixed")
        let natural = CGSize(width: 520, height: 400)
        let tight = CGSize(width: 260, height: 120)
        let fitted = InspectionZoom.contentScale(natural: natural, available: tight)
        check(fitted < 1 && natural.width * fitted <= tight.width + 0.001 && natural.height * fitted <= tight.height + 0.001, "content scales inside the window")
        check(InspectionZoom.contentScale(natural: natural, available: natural) == 1, "matching window uses the natural card")
        check(NeighborRailPolicy.displayedCount(fitted: 0, lineCount: 5) == 2, "short window still shows the previous and next tickets")
        check(NeighborRailPolicy.displayedCount(fitted: 0, lineCount: 2) == 1, "one other ticket stays visible")
        check(NeighborRailPolicy.displayedCount(fitted: 6, lineCount: 4) == 3, "neighbor budget never exceeds the other tickets")
        let pair = NeighborRailPolicy.indices(count: 5, selectedIndex: 2, visibleCount: NeighborRailPolicy.displayedCount(fitted: 0, lineCount: 5))
        check(pair.previous == [1] && pair.next == [3], "the adjacent ids are the scroll destinations")
