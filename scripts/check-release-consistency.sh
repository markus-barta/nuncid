...
for stem in pinned-card inspection-unpinned inspection-zoom30 inspection-zoom300 workflow-run settings-scanning settings-markers settings-pinned settings-appearance version-history-dark social-preview; do
  [[ -f "$repo_dir/docs/screenshots/$stem-$version.png" ]] || {
    print -u2 "Missing docs/screenshots/$stem-$version.png"
    exit 1
  }
